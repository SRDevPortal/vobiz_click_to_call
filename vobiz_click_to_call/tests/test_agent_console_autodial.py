...
class TestAgentConsoleAutoDial(unittest.TestCase):
    ...
    def test_lead_queue_shows_lead_owner_column(self):
        ...
        self.assertIn("? 16 : 13", colspan_source)
    ...
    def test_agent_console_missed_calls_are_inbound_only(self):
        ...
        self.assertIn("and `direction` = 'Incoming'", missed_source)
        self.assertIn("and `status` in %(statuses)s", missed_source)
        self.assertEqual(missed_source.count("and `direction` = 'Incoming'"), 2)
        self.assertIn("count(*) as call_count", missed_source)
        self.assertIn("group by reference_key", missed_source)
    ...
    def test_analytics_api_is_enabled_with_emergency_kill_switch(self):
        source = CONSOLE_API.read_text(encoding="utf-8")

        self.assertIn("ANALYTICS_API_ENABLED = True", source)
        self.assertIn(
    ...
    def test_agent_desk_activity_tracks_online_time_without_call_availability(self):
        ...
        self.assertIn("ACTIVITY_HEARTBEAT_MS = 60 * 1000", availability)
    ...
    def test_ai_mode_hides_manual_disposition_prompt(self):
        ...
        self.assertIn("if (this.state.ai_disposition_enabled) {", dialog_source)
    ...
    def test_mapping_drives_lead_or_patient_queue(self):
        ...
        self.assertIn('"fieldname": "sr_dpt_language"', mapping_json)
        ...
        self.assertIn("patient_matches_mapping(row, agent or {})", console)
    ...
    def test_agent_console_dashboard_uses_filtered_analytics(self):
        ...
        self.assertNotIn("_apply_visible_crm_lead_scope(filters)", console)
        ...
        self.assertIn("frappe.get_all(", console)
        self.assertNotIn('"crm_lead_reference_names"', console)
        self.assertIn('filters["lead_owners"]', console)
        self.assertIn('filters["teams"]', console)
        self.assertIn("exists (", console)
        ...
        self.assertIn('"agents": _analytics_agents_sql(', console)
        self.assertIn("status_filter=status_filter", console)
        ...
        self.assertIn('"status_breakdown": _analytics_status_breakdown(summary, outcome_breakdown)', console)
        self.assertIn('"outcome_breakdown": outcome_breakdown', console)
        ...
        self.assertIn('data-role="team-dropdown"', analytics)
        ...
        self.assertIn('data-role="daily-chart"', analytics)
        ...
        self.assertIn("calls_only: 1", analytics)
        ...
        self.assertIn('data-role="agent-dropdown"', analytics)
